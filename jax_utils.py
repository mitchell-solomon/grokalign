...
def compute_jacobian_norm(apply_fn, params, x, num_projections, rng):
    def model_fn(inputs):
    ...
    rngs = jax.random.split(rng, num_projections)
    v = jax.vmap(lambda key: jax.random.normal(key, (batch_size, output_dim)))(rngs)
    v = v / jnp.clip(jnp.linalg.norm(v, axis=-1, keepdims=True), 1e-8)
    ...
    norm_sum = jnp.sum(jax.vmap(jvp_single)(v), axis=0)
    scaling = output_dim / num_projections
    return jnp.sqrt(jnp.clip(norm_sum * scaling, 1e-8))


def grokalign_loss(apply_fn, params, x, rng, lambda_jac, num_projections=1):
    jacobian_norm = compute_jacobian_norm(apply_fn, params, x, num_projections, rng)
    return lambda_jac * jnp.mean(jacobian_norm)
